...
Example: [ "true,192.168.1.0/16,192.168.1.1,lan",
...
valid RTC device path, or empty string ("") for auto-discovery
...
Example 1: "+127.0.0.1,+[::1]" ---&gt; deny all access, except from 127.0.0.1 and
::1,
...
=== "CLI"
    ```shell
    sudo pihole-FTL --config webserver.headers=[
    ```
=== "Environment (Docker Compose)"
    ```yaml
    environment:
