...
Generate a token with `POST /api/auth/prometheus`. This request itself uses the regular API authentication, i.e., you need a valid session ID (`SID`), see [Authentication](auth.md).

<!-- markdownlint-disable code-block-style -->
???+ example "Generate a scrape token"
...
The new hash is written to `/etc/pihole/pihole.toml` automatically. If your configuration file is read-only, the token still works until the next restart of `pihole-FTL` and a warning is logged.
...
???+ success "Success response"
    ...
    ```text
    # HELP pihole_queries Number of DNS queries in FTL's history window (misc.maxHistory)
...
Setting `webserver.api.prometheus.token` back to an empty string disables the endpoint again - `GET /api/metrics` then returns `404 Not Found`, just as on an installation that never enabled it.

<!-- markdownlint-disable code-block-style -->
???+ example "Disable the metrics endpoint"
...
You can also edit `/etc/pihole/pihole.toml` directly (and restart `pihole-FTL` afterwards) or change the setting in the web interface.

## Configuration options

Setting | Default | Description
--------|---------|------------
`webserver.api.prometheus.token` | `""` | SHA-256 hash of the scrape token. Empty disables the endpoint. Set by `POST /api/auth/prometheus`, see above.
