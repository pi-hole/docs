...
Using Nginx is not just about redirection or SSL; it unlocks professional-grade features that go beyond the native capabilities of Pi-hole:

*   **Enhanced Security:** Implement **Basic Auth** or **Client Certificates (mTLS)** to lock down the web interface and API before user even reaches Pi-hole.
*   **Protocol Control:** Fine-tune modern protocols like **HTTP/2** or **HTTP/3 (QUIC)** and manage complex WebSocket handshakes for real-time dashboard updates.
*   **Header Manipulation:** Adopt custom security headers (HSTS, CSP, X-Frame-Options) to harden your instance against cross-site scripting or clickjacking.
*   **Infrastructure Synergy:** Allow to integrate seamlessly Pi-hole into an existing stack of services under a single domain alongside a **WAF (Web Application Firewall)** for active threat mitigation.
...
To avoid conflicts with `Nginx` or other services running on your system, we need to move Pi-hole v6 away from the standard HTTP port (80). In this guide, we will use **8053** as the internal port for Pi-hole, while `Nginx` will handle the external secure traffic on port **453** (or any other port you prefer).

Additionally, we will configure an internal SSL port, **8054**, allowing Pi-hole to manage its own certificate renewal process locally. We will also set up port **8080** to demonstrate how to automatically redirect HTTP traffic to the secure HTTPS port.
...
You must instruct Pi-hole to stop listening on port 80, 443 or any other ports and switch to our (arbitrary choosen) port **8053**. Additionally, for security, we will restrict via built-in [Access Control List (ACL)](/ftldns/configfile/?h=acl#acl) its web interface to listen only on the local interface (`127.0.0.1`), ensuring it is only accessible via the `Nginx` proxy.

You can apply these changes using the `pihole-FTL` command line configuration tool:
```bash
# Set the custom ports
sudo pihole-FTL --config webserver.port 8053,8054s

# Decrease SSL certificate regeneration validity to a lower value to increase security
sudo pihole-FTL --config webserver.tls.validity 7
...
1. **Create the proxy file configuration:**
```bash
...
map $http_upgrade $pihole_connection_upgrade {
    default upgrade;
    ''      "";
}
...
server {
    ...
    # --- SSL/TLS ---
    # Target the SSL/TLS certificate generated by Pi-hole
    ssl_certificate /etc/pihole/tls.pem;
    ssl_certificate_key /etc/pihole/tls.pem;
    ...
    location / {
        ...
        # WebSocket support (For Pi-hole v6 real-time updates)
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection $pihole_connection_upgrade;
...
3. **Enable the proxy, test and restart `Nginx`:**
```bash
...
In this guide, as explained the *cheat* in [Ports layout ](#ports-layout), we configure `Nginx` to use the certificate automatically generated and managed by Pi-hole v6. This ensures that the communication remains encrypted without requiring you to manually generate or renew certificates for the proxy. If you prefer to use your own custom certificates, customize the configuration accordingly.
...
- **X-Frame-Options (`SAMEORIGIN`)**: Prevents the Pi-hole interface from being loaded inside an `<iframe>` on external sites. This protects users from **Clickjacking** attacks, where an attacker might overlay an invisible frame to trick you into clicking buttons on your dashboard without your knowledge.
- **X-XSS-Protection (`1; mode=block`)**: Enables the anti-XSS (Cross-Site Scripting) filter built into most modern browsers. If a malicious script injection is detected, the browser will block the entire page from loading rather than attempting to "sanitize" it.
- **X-Content-Type-Options (`nosniff`)**: Prevents browsers from bypassing the declared MIME type (MIME-sniffing) to guess the file content. This stops attacks where a harmless-looking text or image file is interpreted and executed as a malicious script (JavaScript).
...
##### The WebSocket Proxy Directives
Pi-hole v6 relies heavily on **WebSockets** to provide real-time updates on the dashboard (such as live query graphs and statistics) without requiring you to refresh the page. Inside the `location` block, these three lines handle them between your browser and Pi-hole:

- **`proxy_http_version 1.1;`:** WebSockets (between `Nginx` and Pi-hole) require HTTP/1.1 to function correctly, not the basic HTTP1;
- **`proxy_set_header Upgrade $http_upgrade;`:** This passes the "Upgrade" signal from the browser to the Pi-hole backend;
- **`proxy_set_header Connection $pihole_connection_upgrade;`:** This uses the result of the `map` block to tell Pi-hole whether to treat this as a persistent WebSocket connection or a standard one.

By default, `Nginx` does not forward WebSocket connections, so we it is a good practice to explicitly configure it to "upgrade" standard HTTP requests in a smart way thanks to `map` block, otherwise we may experience issues on **Update Gravity** page. The block acts as a logical switch:

- It looks at the `$http_upgrade` header sent by the browser;
- If the header is present, it sets the connection to `upgrade`;
- If the header is empty (a standard request), it sets it to an empty string `""`; we must not set this to "close" by default otherwise it would terminate the connection immediately after each request, rendering the `keepalive` directive we configured earlier completely useless.

!!! tip "Why is this necessary?"
    Without this configuration, the Pi-hole v6 dashboard will still load, but you may experience issues in the live statistics and real-time logs, as for example in **Update Gravity** page.
...
3. **Generate the Client Certificate**
    We generate the private key for the user and a Certificate Signing Request (CSR); in real world, we should send the generated CSR  to the CA for the signing, getting back the Certificate; in this guide, and in Private PKI environments, we autonomusly sign it with our Private CA.
...
If you encounter errors specifically when clicking "Save" or "Update" within the Pi-hole settings pages, it is likely that the request body exceeds the configured `Nginx` limit. Verify that `client_max_body_size` is set to at least `2M`. Refer to the [Error Resolution (413 & 414)](#error-resolution-413-414) section for more information.

### Query Log issues (Error 414)
If you experience **414 (Request-URI Too Large)** errors, see blank logs, or get continuous error pop-ups when accessing the **Query Log** (or clicking on dashboard graphs), ensure your `large_client_header_buffers` is correctly set. This directive is essential for handling the long URLs generated by Pi-hole's filtering system. Refer to the [Error Resolution (413 & 414)](#error-resolution-413-414) section for more information.

### Real-time dashboard update issues
Pi-hole v6 uses WebSockets for real-time dashboard updates (e.g., live query logs and status graphs). If you notice that the statistics do not refresh automatically or you see "Connection Lost" messages, ensure that the `Upgrade` and `Connection` headers, along with the `map` block, are correctly configured as described in the [The WebSocket Proxy Directives ](#the-websocket-proxy-directives) section. These headers allow the persistent connection required for real-time data streaming.
