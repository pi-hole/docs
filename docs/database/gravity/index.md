...
In addition to the ability to add comments to individual domains, we also offer a powerful way of managing domains through groups. Each domain can either be not added to a group ("individual"), added to exactly one group ("grouped") or can even be added and managed through multiple groups ("multi-grouped"). See [domain group management](groups.md) for further details.

## Gravity Table (`gravity`)
The `gravity` table consists of the domains that have been processed by Pi-hole's `gravity` (`pihole -g`) command. The domain in this list are the unique collection of domains sources from the configured sources (see the [`adlist` table](lists.md#adlist-table-adlist)).
